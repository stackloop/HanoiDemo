...
class Hanoi(Scene):
    ...
    def move_block(self, from_stack, to_stack):
        ...
        self.play(Transform(block, block.copy().to_edge(UP)), run_time=0.5)

        target_x = (LEFT * self.stack_distance) + (RIGHT * self.stack_distance * to_stack)
        move_x = target_x - block.get_center()[0]
        self.play(block.animate.shift(RIGHT * move_x), run_time=0.5)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if size < 1 or size > 50:
            error("Size must be a value in the range 1 - 50.")
